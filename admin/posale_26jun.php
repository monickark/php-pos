...
<div id="testdiv" style="display:none">
            ...
            <input type="submit" value="submit" name="submit" class="btn btn-primary  btn-round" style="float: right" value="Submit Order" /> 
...
sno++;
        ...
        var updrow ='<tr class="even pointer" id="final"><td><input type="hidden" id="ident'+sno+'" name="sid[]" value="'+proid+'">&nbsp;'+pname+' &nbsp;'+pcolor+' &nbsp; '+pcapcty+'<input type="hidden" name="pname[]" value="'+pname+'"><input type="hidden" name="proid[]" value="'+proid+'"> &nbsp;<div style="float: left;margin-top: 5px;" id="price'+proid+'" value="'+price+'"> </div>&nbsp; </td><td>     <input type="hidden" id="imei'+proid+'" name="imei[]" value="'+imeino+'">'+imeino+'</td><td> <input type="text" id="disc'+proid+'" name="pqty[]" class="discamt form-control" onchange="findtot('+proid+');" size="30" readonly value="'+discount+'" " class="form-control col-md-2"><a id="editdis" href="javascript:void(0)" onclick="editdisc('+proid+');" class="btn btn-sm btn-icon btn-default btn-outline btn-round"><i class="icon wb-pencil" aria-hidden="true"></i></a></td><td>&nbsp;<div style="float: left; " id="tot'+proid+'">$. '+aftdis+'<br><span class="price">$. '+price+'</span></div><input type="hidden" name="itemtot[]" id="itemtot'+proid+'" value="'+aftdis+'" class="itemtotal">&nbsp; </td><td>&nbsp; <a id="'+proid+'" href="javascript:void(0);" class="remCF btn btn-round btn-outline btn-danger btn-sm">Remove</a></td></tr>'; 
...
$("#disconcar").html(totdis - disval);

/* Have to work on Two Submits on click*/
nops=$("#prodnos_h").val();
nops=parseInt(nops);
alert(nops);
...
$("#netpay").html(nettot+parseFloat(disval));
...
$("#disc"+disval).removeAttr("readonly"); 
...
$.ajax({
  ...
  type: "POST",
      url: "getsubproduct.php",
        data:{brd1: proid,brd2 : proid2,colid:color },
      success: function(data){
