...
echo "<script> alert('".$pdtdis."')</script>";
echo "<script> alert('".$itmtot."')</script>";

echo "<script> alert('".$imei."')</script>";

echo "<script> alert('".$typeofsale."')</script>";
...
if($ite['itemname']=="")
{
   $itemnames=$_POST['pname'][$i];
}
else
{
$itemnames=$ite['itemname'];
}
...
echo "<script> alert('".$typeofsale."')</script>";*/
 ...
 $salesitem=array(
                ...
                "itemprice"      =>     mysqli_real_escape_string($data->con, $itemamt),
                "itemname"       =>     mysqli_real_escape_string($data->con, $itemnames),
...
/*
 echo "<script> alert('".$update_inventory."')</script>";*/
  ...
  $update_imei_id = array(
      "itmimei"      =>    mysqli_real_escape_string($data->con, $imei)
    );
...
/*     
    header("Location:salesorder.php?flag=succ&type=sales");*/
     ...
     echo '<script>window.location.href = "test.php?print_id='.$insid.'"</script>';
  }
else
{
    ...
    <div class="row">
     ...
     <div class="panel-body panel-collapse"> 
            ...
            <div class="input-search input-search-dark" style="margin:0 !important;max-width: 100%;border-radius: 0px !important;">
              ...
              <input class="form-control" id="keyword" name="search" placeholder="Start typing product name or scanning..." type="text" style="padding-left:5.073rem" autocomplete="off" onkeypress="handle(event)">
...
$categor=mysqli_query($conn,'SELECT * FROM category');
while($cat=mysqli_fetch_array($categor))
{

$bnrand=mysqli_query($conn,'SELECT * FROM items WHERE category = "'.$cat['cid'].'"');

if(mysqli_num_rows($bnrand)!=0)
{
if(in_array($cat['Category_name'],$categs)==0)

{
  array_push($categs,$cat['Category_name']);
...
<!-- 
onclick="cate(this.value)" -->


<li class="moose" value="<?php echo $cat['cid'];?>" onclick="brand(this.value)">
              <div class="panel">
                <figure class="overlay overlay-hover animation-hover">
                  ...
                  <figcaption class="overlay-panel overlay-background overlay-fade text-center vertical-align" id="access" data-value="<?php echo $cat['cid'];  ?>">
                           <?php echo $cat['Category_name'];  ?>             
                  </figcaption>
                </figure>
                <div class="text-truncate "> <center><?php echo $cat['Category_name']  ?></center></div>
              </div>
            </li>
...
<div id="customer_names"></div>
                      ...
                      <div class="step col-md-2 cjk " data-target="#exampleFormModal" data-toggle="modal" onclick="validate_cust()">
                       <!--  <span class="step-icon icon wb-user-add" aria-hidden="true"></span> -->
                        <div class="step-desc">
                          <span class="step-title">Add Customer</span>
                        </div>
                      </div>
                      <div class="step col-md-2 cjk" data-target="#exampleFormModal1" data-toggle="modal" onclick="note_valid()">
                      <!--   <span class="step-icon icon wb-time" aria-hidden="true"></span> -->
                        <div class="step-desc">
                          <span class="step-title">Add Notes</span>
                        </div>
                      </div>
                          <div class="step col-md-2 cjk" data-target="#exampleFormModal2" data-toggle="modal" >
                       <!--  <span class="step-icon icon wb-time" aria-hidden="true"></span> -->
                        <div class="step-desc">
                          <span class="step-title">Other Brands</span>
                        </div>
                      </div>
              ...
              <div class="nav-tabs-horizontal nav-tabs-inverse" data-plugin="tabs">
                <ul class="nav nav-tabs" role="tablist">
                  ...
                  <div class="tab-pane" id="exampleTabsInverseTwo" role="tabpanel">
                   ...
                   data-min="0" data-max="10000" data-step="0.1" data-decimals="2"
                      data-boostat="5" data-maxboostedstep="10" data-prefix="$" value="0" />
                      <br>
                      <button class="popup-modal-dismiss btn btn-primary btn-outline" type="button" onclick="netdiscount(2);" data-dismiss="modal-dismiss">Apply Discount</button>
...
var tax=parseFloat(total/((taxrate/100)+1)).toFixed(2);

/*var tax=total/((10/100)+1);*/
var tax_diff=parseFloat(total-tax).toFixed(2);


tax=tax.toFixed(2);
tax_diff=tax_diff.toFixed(2);
                      ...
                      <div class="modal-dialog modal-simple">
                        <form class="modal-content" name="add customer" id="addnewcusr">
                          ...
                          <div class="modal-body">
                            <div class="row">
                              <div class="col-xl-4 form-group cd">
                                <input type="text" id="name" class="form-control cds" name="firstName" placeholder="Name" >
                              </div>

                              <div class="col-xl-4 form-group cd">
                                <input type="email" id="email" class="form-control cds" name="email" placeholder="Your Email" >
                              </div>
                             <div class="col-xl-4 form-group cd">
                                <input type="text" id="mobile" class="form-control cds" name="mobile" placeholder="Mobile Number"  >
                              </div>
...
<button class="btn btn-primary btn-outline" id="add_butt" onclick="gos()" type="button">Add Customer</button>
                              </div>
                            </div>
                          </div>
                        </form>
  ...
  $('#addnewcusr').validate({ // initialize the plugin
      ...
      minlength:10
        ...
        highlight: function ( element, errorClass, validClass ) {
          ...
          $( element ).parents( ".cds" ).addClass( "has-error" ).removeClass( "has-success" );

        },

        unhighlight: function (element, errorClass, validClass) {

          $( element ).parents( ".cds" ).addClass( "has-success" ).removeClass( "has-error" );
...
function gos()
{
  var x=0;
var name=$('#name').val();
   var reqlength = $('.cds').length;
    /*console.log(reqlength);*/
    var value = $('.cds').filter(function () {
        return this.value != '';
    });
...
{
 var y=confirm("Some fields are Empty.Leave Now??");
 ...
 function addnotes()
{
 ...
 <div class="modal fade" id="exampleFormModal2" aria-hidden="false" aria-labelledby="exampleFormModalLabel"
                      role="dialog" tabindex="-1">
                       <div class="modal-dialog modal-simple">
                        <form class="modal-content" name="add_product" id="addnewpdts">
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">×</span>
                            </button>
                            <h4 class="modal-title" id="exampleFormModalLabel">Other Brands Entries</h4>
                          </div>
                          <div class="modal-body">
                            <div class="row">
                              <div class="col-xl-4 form-group pds">
                                <input type="text" id="pdt_name" class="form-control cds" name="pdt_name" placeholder="Product Name" >
                              </div>

                              <div class="col-xl-4 form-group pds">
                                <input type="email" id="pdt_qty" class="form-control cds" name="pdt_qty" onkeypress="return isNumber(event)"  placeholder="Product Qty" >
                              </div>
                             <div class="col-xl-4 form-group pds">
                                <input type="text" id="pdt_price" class="form-control cds" name="pdt_price" onkeypress="return isNumber(event)"  placeholder="Price"  >
                              </div>

   <div class="col-xl-12 form-group">
<input type="text" id="tot_pdt_amt" class="form-control cds" name="tot_pdt_amt" disabled >
  </div>
  <div class="col-md-12 float-right">
<button class="btn btn-primary btn-outline" id="add_prod"  onclick="add_new_prod()" type="button">Add Product</button><span id="error_tab"></span>
                              </div>
                            </div>
                          </div>
                        </form>
...
else
{
  ...
  var price=$("#tot_pdt_amt").val(); 

var single=price/qty;
...
var typoes="Manual";
 ...
 var updrow ='<tr class="even pointer" id="final"><td><input type="hidden" id="ident'+sno+'" name="sid[]" value="'+proid+'">&nbsp;'+name+' &nbsp;  X &nbsp;'+qty+'<input type="hidden" name="pname[]" value="'+name+'"><input type="hidden" name="proid[]" value="'+proid+'"> &nbsp;<div style="float: left;margin-top: 5px;"><input type="hidden" id="price'+proid+'" name="itemtot[]" value="'+price+'" ><input type="hidden" id="qty'+proid+'" name="pdtqty[]" value="'+qty+'" ><input type="hidden" id="sing'+proid+'" name="pdtamt[]" value="'+single+'" ></div>&nbsp; </td><td> </td><td>&nbsp;<div style="float: left; " id="tot'+proid+'">$. '+price+'&nbsp; <input type="hidden" name="typoes[]" value="'+typoes+'"></td><td>&nbsp; <a id="'+proid+'" href="javascript:void(0);" class="remCoF btn-danger btn btn-sm btn-icon btn-default btn-outline btn-round"><i class="icon wb-close" aria-hidden="true"></i></a></td></tr>'; 
$("#upd-row").append(updrow);


/*var prodno=$("#pdt_name").val(); 
var subtot=$("#pdt_name").val(); 
var rate=$("#pdt_name").val(); 
var tot_disc=$("#pdt_name").val(); 
var net_pay=$("#pdt_name").val(); */



 var prodno=$("#prodnos_h").val(); 
 
prodno=parseInt(prodno)+parseInt(qty);

$("#prodnos_h").val(prodno); 
...
var tax=parseFloat(subtot/((taxrate/100)+1)).toFixed(2);
var tax_diff=parseFloat(subtot-tax).toFixed(2);
...
var tot_disc=$("#disconcar").text(); 

/* alert(tot_disc);*/
 tot_disc=parseFloat(tot_disc);

 $("#netpay").html(subtot-tot_disc);
...
/*$("#exampleFormModal2").modal('toggle');*/
$('#exampleFormModal2').css('display', 'none');
...
prevdis=parseFloat(prevdis);
/*alert(prevdis);
alert(pridis);*/

pridis=parseFloat(pridis);
...
var tax=parseFloat(totdis/((taxrate/100)+1)).toFixed(2);
var tax_diff=parseFloat(totdis-tax).toFixed(2);
...
// $("#ajax_response").
  console.log(barcode); return false;
...
/*alert(msg);*/
 ...
 $("#ajax_response").fadeIn("slow"); 
            $("#ajax_response").html('<div style="text-align:;">Processing...</div>');
          }
          else
          {
            $("#ajax_response").fadeIn("slow"); 
...
var prev_subtot=$("#subtotal_h").val();

/*alert(prev_subtot);*/

prev_subtot=parseFloat(prev_subtot);
...
/**/
/*var net_dis=$("#disconcar").html(totdis);*/
...
$("#netpay").html(nettot+prev_subtot-tot_disc);
...
alert(cate);*/
 $.ajax({
           type: "POST",
           url: "getsubproduct_ui.php",
           data: {data: val,tpe:type,cat:cate},
           success: function(msg){  
          /*  alert(msg);*/
...
$(".pds").bind("keyup change", function(e) {

var val1= $("#pdt_qty").val(); 
var val2= $("#pdt_price").val(); 
